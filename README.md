# Плейлисты Rutube
Плейлист Rutube - виджет для воспроизведения предопределенной группы роликов с сайта rutube.ru на сторонних ресурсах. Создать плейлист Rutube и разместить его на своем сайте или блоге может любой пользователь интернета.
...
Для создания плейлиста, содержащим несколько источников, необходимо добавить их к списку `"feeds"`:
```json
{
    "feeds": [
        { "id": "28", "type": "tv" },
...
Помимо этого, источники с типом `tv` (ТВ-шоу) поддерживают группировку по сезонам, котороые можно передать в параметре `"params"`:
```json
{
    "feeds": [{
        "id": "28",
